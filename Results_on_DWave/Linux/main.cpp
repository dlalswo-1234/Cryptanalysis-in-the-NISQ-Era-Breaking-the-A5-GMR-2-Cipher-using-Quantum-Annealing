...
int Table[8][256][2], color[8][256], bv[8][256], bvsz[8];
// Current vertex, Next Vertex
int ci, cj, ni, nj;
// For Pre-filtration (Queue)
int qi[2048], qj[2048], qf, qb, qs, vs[8][256];
// For Table Analysis (Stack)
int si[8], sj[8], st, ss[8], sk[8], sss[8], sbvj[8];
// For Verification
bool KC[8][8]; long long Vcnt;
// Result of Cryptanalysis
vector<uint64_t> recovered;
...
int main(int argc, char* argv[]) {
    ...
    Vcnt = 0;
    for (int i = 0; i < 7; i++) for (int j = 0; j < 256; j++) bv[i][j] = j, color[i][j] = BLACK;
    for (int i = 0; i < 7; i++) bvsz[i] = 256;
    ...
    ofstream fout("e_matrix.txt");
    for (int i = 0; i < tot_ind; i++) {
        for (int j = 0; j < tot_ind; j++) {
            fout << e_matrix[i][j];
            if (j != tot_ind - 1) fout << " ";
    ...
    /* for(auto x:e[197]) cout << x << ' ';
    cout << '\n';
    for(auto x:e[477]) cout << x << ' ';
    cout << '\n';
    for(auto x:e[693]) cout << x << ' ';
    cout << '\n';
    for(auto x:e[968]) cout << x << ' ';
    cout << '\n';
    for(auto x:e[1170]) cout << x << ' ';
    cout << '\n';
    for(auto x:e[1437]) cout << x << ' ';
    cout << '\n';
    for(auto x:e[1749]) cout << x << ' ';
    cout << '\n';
    for(auto x:e[1940]) cout << x << ' ';
    cout << '\n'; */
    ...
    /* cout << '\n';
    for(auto x:reachable[197]) cout << x << ' ';
    cout << '\n';
    for(auto x:reachable[477]) cout << x << ' ';
    cout << '\n';
    for(auto x:reachable[693]) cout << x << ' ';
    cout << '\n';
    for(auto x:reachable[968]) cout << x << ' ';
    cout << '\n';
    for(auto x:reachable[1170]) cout << x << ' ';
    cout << '\n';
    for(auto x:reachable[1437]) cout << x << ' ';
    cout << '\n';
    for(auto x:reachable[1749]) cout << x << ' ';
    cout << '\n';
    for(auto x:reachable[1940]) cout << x << ' ';
    cout << '\n'; */
...
void base() {
    ...
    for (int i = 0; i < 256; i++) bv[7][i] = i, color[7][i] = BLACK;
    bvsz[7] = 256;
}

// option 0-> Initialize all variables
// option 1-> Turn black vertices after Pre-filtration with first keystream frame to white
void init(int option) {
    if (!option) {
        for (int i = 0; i < 7; i++) for (int j = 0; j < 256; j++) color[i][j] = WHITE;
        recovered.clear();
    }
    else {
        for (int i = 0; i < 7; i++) for (int j = 0; j < 256; j++) color[i][j] = color[i][j] == BLACK ? WHITE : DEAD;
    }
    for (int i = 0; i < 7; i++) bvsz[i] = 0;
}
